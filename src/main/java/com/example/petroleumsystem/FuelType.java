...
public class FuelType implements Initializable {
    ...
    @FXML
    private TextField txtLitterPrice;

    @FXML
    private TableColumn<FuelTypeClass, Double> colLitterPrice;
    ...
    void getData(MouseEvent event) {
        ...
        txtLitterPrice.setText(String.valueOf(fuelTypeClass.getPrice_per_litters()));
    }
    ...
    void onEdit(ActionEvent event) {
        try {
            if(txtfuelType.getText().isEmpty()){
                ...
                double price_per_litters = Double.parseDouble(txtLitterPrice.getText());
                PreparedStatement ps = db.connection.prepareStatement("update fuelType set fuel_type = ? , price_per_litters = ?  where id = ?");
                ps.setString(1,fuel_type);
                ps.setDouble(2,price_per_litters);
                ps.setInt(3,id);
                ps.executeUpdate();
                Alert alert = new Alert(Alert.AlertType.INFORMATION);
    ...
    void FetchData() throws SQLException {
        ...
        colLitterPrice.setCellValueFactory(new PropertyValueFactory<FuelTypeClass , Double>("price_per_litters"));
